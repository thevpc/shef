...
//                                    l.setText(null);
    ...
    private class ResizeHandler extends MouseInputAdapter {
        ...
        private void replace(Element elem, String html) {
            ...
            ShefHelper.beginCompoundEdit(document);
            try {
                document.setOuterHTML(elem, html);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            ShefHelper.endCompoundEdit(document);
        }
    }
    ...
    private class ResizableView extends DelegateView {
         ...
         *
         * @param p
         * @return one of NW, SW, NE, SE, N, S, E, W or -1 if a handle is not at
         * the point
         */
     ...
     * <p>
     * The default view for such {@link Element}s are big ugly blocky
     * JTextFields, which doesn't really work well in practice and tends to
     * confuse users. This view replaces the default and simply draws the
     * elements as blocks that indicates the presence of an uneditable
     * {@link Element}.
     * <p>
     *
     * @author Bob Tantlinger
